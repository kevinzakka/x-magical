...
def _is_var_name(name):
    return name not in RESERVED_NAMES and all(
        not name.startswith(p) for p in RESERVED_PREFIXES
    )
...
class PhysicsVariablesBase(metaclass=_PhysicsVariablesMeta, _pbm_skip=True):
    ...
    def defaults(cls):
        ...
        return cls(_var_values={k: v.default for k, v in cls.variables.items()})
